...
def select_features(list_model,data):
    genes = data.columns[1:]
    features_selection = []
    list_genes = {}

    for idx in range(len(list_model)):
        tmp_selection = model_value(list_model[idx], data)
...
def features_selection(genes_coef, pourcent):
    features_selected = []

    for gene in genes_coef.keys():
        if(genes_coef[gene] >= pourcent):
            features_selected.append(gene)

    return features_selected

def model_value(model,data):
    genes_coef = {}
    coefficients = model.feature_importances_
    genes = data.columns[1:]
    pourcent = np.percentile(coefficients , 95)

    for idx in range(len(coefficients)):
        if (coefficients[idx] != 0):
            genes_coef[genes[idx]] = coefficients[idx]

    tmp_selection = features_selection(genes_coef, pourcent)
...
if __name__ == "__main__":
    ...
    list_rf = create_all_model(geneTrain, 1)
    print("model created")
    ...
    ## Features selection
    features_selected = select_features(list_rf, data)
    ...
    ## Export final result
    export_result(logistic, newData, newTest, "out_1model.csv")
