...
def create_fieldnames(list_pam50, clinical_id):
    idx = 0
    list_fieldnames = []

    print("Gene")
    for idx in range(len(list_pam50)):
        if(list_pam50[idx] != ""):
            list_fieldnames.append(list_pam50[idx])
...
def create_value_list(gene_data,clinical_data,clinical_id, list_pam50):
    value_list = []
    value_list.append(create_fieldnames(list_pam50,clinical_id))
    ...
    for patient in gene_data.keys():
        if (patient in list_id):
            for gene in gene_data[patient].keys():
                if (gene in list_pam50):
                    tmp_list.append(gene_data[patient][gene])
            for idx in range(len(clinical_id)):
                tmp_list.append(clinical_data[patient][clinical_id[idx]])
...
if __name__ == "__main__":
    ...
    value_list = create_value_list(gene_data,clinical_data,clinical_id, list_pam50)
    print(value_list[0])

    create_csv(value_list,"../pam50clinical.csv")
